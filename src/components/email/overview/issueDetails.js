...
const IssueDetails = ({ match }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
       ...
       let {data} = await axios.get(`${process.env.REACT_APP_API}/api/v1/issue?id=${match.params.id}`, {headers: {"Authorization" : `Bearer ${userInfo.token}`}})
...
//     const {firstName, lastName, email, country, phone, gender} = user
  ...
  return (
    ...
    <ProjectDetailsWrapper>
      ...
      <div style={{textAlign: 'center'}}><h1><span>Issue Details</span></h1></div>
        <Row gutter={25}>
