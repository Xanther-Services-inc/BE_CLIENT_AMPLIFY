...
const Features = () => {
  ...
  useEffect(() => {
    ...
    <br />
        ...
        <Row>
          {console.log(listData)}
          {listData.length > 8 ? listData.slice(0,8).map((feature) => (
            <Col key={feature.id} xs={12} md={6} lg={3} xl={3}>
              <Link to={`/service/${feature.id}`}>
              <Card style={{ width: '18rem', marginBottom: '1rem'}} className='zoom'>
              <Card.Img style={{ height: '18rem' }} variant="top" src={`https://products-imgs.s3.us-east-2.amazonaws.com/${feature.doc_key}`} />
              <Card.Body>
                <Card.Title>{feature.title}</Card.Title>
                <Card.Text>
                 {feature.desc.length > 25 ? feature.desc.substring(0,25) + '..' : feature.desc}
                </Card.Text>
