  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data } = await Axios.get(
        `${process.env.REACT_APP_API}/api/v1/testimonials`
      );
      console.log(data);
      setTesti(data);
    };
          ...
          <div className="row text-center">
            <div className="col-lg-8 col-md-12 ml-auto mr-auto">
              <div className="section-title">
                ...
                <h6>
                  {dataTestimonial.data.home[0].content.testimonial.title}
                </h6>
                <h2 className="title">
                  {
                    dataTestimonial.data.home[0].content.testimonial[
                      "sub_title"
                    ]
                  }
                </h2>
                ...
                <div className="row align-items-center">
                  <div className="col-lg-7 col-md-12">
                    <div className="carousel-inner">
                      {testi.length > 4
                        ? testi.slice(0, 4).map((member, index) => (
                            <div
                  ...
                  <div className="col-lg-5 col-md-12 md-mt-5">
                    <div className="controls">
                      {dataTestimonial.data.home[0].content.testimonial.anti_holder.map(
                        (holder, index) => (
                          <div className={`anti-${holder}-holder`} key={index}>
                            <div className={`anti-${holder}`}></div>
                          </div>
                        )
                      )}
                      <ul className="nav">
                        {testi.length > 4
                          ? testi.slice(0, 4).map((img, index) => (
                              <li
                                key={index}
